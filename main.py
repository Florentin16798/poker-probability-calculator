...
from computation import prob_win
    ...
    def button_card(self, id_button):
        ...
        if self.val == 53:
            ...
            self.bout = Button(
                self.window,
                image=self.im,
                command=lambda: self.interface.set_card(id_button),
                bd="5",
            )
        if id_button != 7:
            self.bout.grid(row=self.row, column=self.column, ipadx=7, ipady=7)
...
class Interface:
    ...
    def __init__(self, window, int_cards=[52, 52, 52, 52, 52, 52, 52, 52, 52]):
        ...
        self.player_cards = [
            Card(int_cards[0], window, self, 4, 0),
            Card(int_cards[1], window, self, 4, 1),
        ]
        self.opponent_cards = [
            Card(int_cards[2], window, self, 0, 0),
            Card(int_cards[3], window, self, 0, 1),
        ]
        self.flop = [
            Card(int_cards[4], window, self, 2, 0),
            Card(int_cards[5], window, self, 2, 1),
            Card(int_cards[6], window, self, 2, 2),
        ]
        self.turn = [Card(int_cards[7], window, self, 2, 3)]
        self.river = [Card(int_cards[8], window, self, 2, 4)]
        self.cards = (
            self.player_cards + self.opponent_cards + self.flop + self.turn + self.river
        )
        self.window = window
    ...
    def display_buttons(self):
        ...
        """
        Label(self.window, text="Opponent Cards").grid(
            row=1, column=0, sticky="n", columnspan=2, pady=10
        )
        Label(self.window, text="Flop").grid(
            row=3, column=0, sticky="nsew", columnspan=3, rowspan=1, pady=10
        )
        Label(self.window, text="Turn").grid(
            row=3, column=3, sticky="nsew", columnspan=1, rowspan=1
        )
        Label(self.window, text="River").grid(
            row=3, column=4, sticky="nsew", columnspan=1, rowspan=1
        )
        Label(self.window, text="Player Cards").grid(
            row=5, column=0, sticky="nsew", columnspan=2, pady=10
        )
        if self.condition_of_calculation():
            ...
            ).grid(row=2, column=5, sticky="nsew", padx=20, ipadx=7, ipady=15)
        else:
            Button(
                self.window,
                text="Choose player cards \n and at least the 3 flop cards \n or the 2 opponent cards",
            ).grid(row=2, column=5, sticky="nsew", padx=20, ipadx=7, ipady=15)
        for i in range(9):
    ...
    def condition_of_calculation(self):
        ...
        if self.int_cards[0] < 52 and self.int_cards[1] < 52:
            if (
                self.int_cards[4] == 52
                or self.int_cards[5] == 52
                or self.int_cards[6] == 52
            ):
                if (
                    self.int_cards[4] < 52
                    or self.int_cards[5] < 52
            ...
            if (
                self.int_cards[4] < 51
                and self.int_cards[5] < 51
                and self.int_cards[6] < 51
            ):
                return True
    ...
    def compute(self):
        ...
        Label(self.window, text=str(proba_opp) + "%").grid(
            row=0, column=5, sticky="nsew", padx=20, ipadx=7, ipady=15
        )
        Label(self.window, text=str(proba_pla) + "%").grid(
            row=4, column=5, sticky="nsew", padx=20, ipadx=7, ipady=15
        )
...
def choose_card(selected_card, interface):
    ...
    window = Tk()
    card_list = [
        Card(i * 13 + j, window, interface, i, j) for i in range(4) for j in range(13)
    ]
    for i in range(52):
