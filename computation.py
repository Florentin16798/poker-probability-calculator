...
from constants import CARD_PAIRS, CARDS, CARDS_NUM, CARDS_REPR, FIVE_CARD_SETS
...
def category(hist_ranks, hist_suits):
    ...
    if hist_ranks == [
        ...
        0,
        1,
    ]:  # if the straight starts by an Ace
        straight = True
    if flush:
        if straight:
...
def best_cat_feat(seven_cards):
    ...
    """
    cat_feats = [
        category_and_features(hand) for hand in list(it.combinations(seven_cards, 5))
    ]
...
def prob_win_turn(hole_cards, flop_turn):
    ...
    for river in CARDS:
        if not river in known_cards:
            ...
            for card_1, card_2 in CARD_PAIRS:
                if (
                    not card_1 in known_cards
                    and not card_2 in known_cards
                    and not river in [card_1, card_2]
                ):
...
def prob_win_turn_Monte_Carlo(
    hole_cards, flop_turn, proportion=0.1, with_replacement=True
):
    """
    ...
    nb_win = 0
    known_cards = hole_cards + flop_turn
    possible_rivers_hole_cards = [
        (r, list(hc))
        for r in CARDS
        for hc in CARD_PAIRS
        if not r in hc and not hc[0] in known_cards and not hc[1] in known_cards
    ]
    if with_replacement:
...
def prob_win_flop_Monte_Carlo(
    hole_cards, flop, proportion=0.005, with_replacement=True
):
    """
    ...
    possible_turns_rivers_hole_cards = [
        (list(tr), list(hc))
        for tr in CARD_PAIRS
        for hc in CARD_PAIRS
        if not hc[0] in known_cards
        and not hc[0] in tr
        and not hc[1] in known_cards
        and not hc[1] in tr
        and not tr[0] in known_cards
        and not tr[0] in hc
...
def prob_win_flop_opponent_known(hole_cards_1, hole_cards_2, flop):
    ...
    possible_turns_rivers = [
        list(tr)
        for tr in CARD_PAIRS
        if not tr[0] in known_cards and not tr[1] in known_cards
    ]
...
def prob_win_preflop_opponent_known_Monte_Carlo(
    hole_cards_1, hole_cards_2, proportion=0.005, with_replacement=True
):
    """
    ...
    possible_table_cards = [
        list(table_cards)
        for table_cards in FIVE_CARD_SETS
        if not card_1_a in table_cards
        and not card_1_a in table_cards
        and not card_2_b in table_cards
        and not card_2_b in table_cards
    ]
    count = 0
    for i in indices:
